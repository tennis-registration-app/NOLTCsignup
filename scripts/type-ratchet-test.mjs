...
/**
 * TypeScript Ratchet Script — Test Files
 *
 * Runs tsc --noEmit -p tsconfig.test.json and compares error count against
 * baseline. Fails if errors exceed baseline counts.
 *
 * Mirrors type-ratchet.mjs but targets tsconfig.test.json (src/ + tests/).
 * Baseline is tracked separately in typescript-test-baseline.json.
 *
 * Usage:
 *   node scripts/type-ratchet-test.mjs              # Check against baseline
 *   node scripts/type-ratchet-test.mjs --update     # Update baseline with current counts
 *   node scripts/type-ratchet-test.mjs --dry-run    # Show comparison without failing
 *
 * Environment:
 *   TYPESCRIPT_TEST_BASELINE_PATH  Override baseline file path (for testing)
 */
...
const BASELINE_PATH =
  process.env.TYPESCRIPT_TEST_BASELINE_PATH ||
  join(ROOT_DIR, 'config', 'ratchets', 'typescript-test-baseline.json');
...
/**
 * Run typecheck:test via npm script and return the raw output.
 */
function runTypecheck() {
  try {
    const result = execSync('npm run -s typecheck:test -- --pretty false', {
      cwd: ROOT_DIR,
      encoding: 'utf-8',
...
function compareToBaseline(current, baseline) {
  ...
  console.log('TypeScript Ratchet Check (test files)');
  console.log('======================================');
  console.log('');
  console.log(
    `Errors: ${current.errors} (baseline: ${baseline.errors}, delta: ${errorDelta >= 0 ? '+' : ''}${errorDelta})`
  );
  console.log('');
  ...
  if (Object.keys(current.byCode).length > 0 || Object.keys(baseline.byCode || {}).length > 0) {
    ...
    const allCodes = new Set([
      ...Object.keys(current.byCode),
      ...Object.keys(baseline.byCode || {}),
    ]);
...
// Main execution
console.log('Running tsc --noEmit -p tsconfig.test.json...');
...
if (errorDelta > 0) {
  console.log('❌ FAIL: Type error count in test files increased!');
  console.log(`   Fix ${errorDelta} error(s) or update baseline if intentional.`);
  process.exit(1);
}

if (errorDelta < 0) {
  console.log('✅ PASS: Type error count in test files improved!');
  console.log('   Consider running with --update to capture new baseline.');
} else {
  console.log('✅ PASS: Type error count in test files within baseline.');
}
