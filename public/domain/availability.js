(function() {
  ...
  function getCourtStatuses({ data, now, blocks, wetSet, upcomingBlocks = [], showingOvertimeCourts }) {
    const Av = window.Tennis.Domain.availability || window.Tennis.Domain.Availability;
    const info = Av.getFreeCourtsInfo({ data, now, blocks, wetSet });
    ...
    // If showingOvertimeCourts not provided, fall back
    // to old logic (shouldn't happen in practice)
    if (showingOvertimeCourts === undefined) {
      showingOvertimeCourts = !hasTrueFree;
    }
    ...
    for (let n = 1; n <= total; n++) {
      ...
      const selectable = (!isWet && !isBlocked) &&
                         ((status === 'free') || (status === 'overtime' && showingOvertimeCourts && !isTournament));
