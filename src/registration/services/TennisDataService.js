...
// Re-export for consumers that need config
    ...
    try {
      ...
      for (const block of filteredBlocks) {
        ...
        if (blockStart < checkTime && blockEnd > windowStart) {
          ...
          console.log(`DEBUG: Court ${courtNumber} block overlaps, check time moved to:`, new Date(checkTime).toLocaleTimeString());
        } else if (blockStart >= checkTime) {
          console.log(`DEBUG: Court ${courtNumber} gap found before block, available at:`, new Date(checkTime).toLocaleTimeString());
          break;
        }
      }
