...
export function getCourtBlockStatus(courtNumber) {
  ...
  try {
    let blocks: Array<{ courtNumber: number; isWetCourt?: boolean; startTime: string; endTime: string; reason?: string; title?: string; eventDetails?: unknown }> = [];
    try {
      blocks = (readJSON(STORAGE.BLOCKS) as typeof blocks) || [];
    } catch {
 ...
 */
type Block = { courtNumber: number; isWetCourt?: boolean; startTime: string; endTime: string; reason?: string; title?: string; eventDetails?: unknown };
