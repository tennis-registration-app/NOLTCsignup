...
export class TennisDirectory {
  ...
  async searchMembers(query) {
    ...
    return ((response.members as Array<Record<string, unknown>>) || []).map((m) => this._normalizeMember(m));
  }
  ...
  async getMembersByAccount(memberNumber) {
    ...
    const members = ((response.members as Array<Record<string, unknown>>) || []).map((m) => this._normalizeMember(m));
