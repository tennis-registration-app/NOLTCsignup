...
import { validateBoardResponse } from '../schemas/apiEnvelope';
import { validateBoard } from '../schemas/domain';
import { logger } from '../logger';

// Use existing ApiAdapter for actual HTTP calls
// This will be imported by the consuming code that sets it up
let apiAdapter: { get(url: string): Promise<Record<string, unknown>> } | null = null;

/**
 * Set the API adapter instance
 * @param {Object} adapter - ApiAdapter instance
 */
export function setApiAdapter(adapter: { get(url: string): Promise<Record<string, unknown>> }) {
  apiAdapter = adapter;
}
...
export async function getBoard() {
  ...
  if (!raw?.ok) {
    const rawMsg = (raw as { message?: string } | undefined)?.message;
    logger.error('BoardApi', 'Failed to fetch board data', rawMsg);
    throw new Error(rawMsg || 'Board fetch failed');
  }
