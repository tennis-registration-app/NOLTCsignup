...
import { STORAGE, COURT_COUNT, SCHEMA_VERSION } from './constants';
...
export function readDataSafe(courtsCount = COURT_COUNT) {
  try {
    ...
    const safe: Record<string, unknown> & { courts?: unknown[]; waitingGroups?: unknown[]; recentlyCleared?: unknown[]; waitlistPromotions?: unknown[] } = {};
    const d = data as Record<string, unknown> | null | undefined;
    safe.courts = Array.isArray(d?.courts)
      ? (d!.courts as unknown[])
      : Array.from({ length: courtsCount }, () => ({ history: [], current: null }));

    safe.waitingGroups = Array.isArray(d?.waitingGroups) ? (d!.waitingGroups as unknown[]) : [];
    safe.recentlyCleared = Array.isArray(d?.recentlyCleared) ? (d!.recentlyCleared as unknown[]) : [];

    // Promotions: make present (read-only default) without mutating storage
    safe.waitlistPromotions = Array.isArray(d?.waitlistPromotions)
      ? (d!.waitlistPromotions as unknown[])
      : [];
