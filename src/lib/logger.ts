 ...
 * @returns {boolean}
 */
function shouldLog(level: keyof typeof LOG_LEVELS) {
...
export const logger = {
   ...
   */
  debug(module: string, message: string, data: unknown = null) {
    if (shouldLog('debug')) {
   ...
   * @param {*} [data] - Optional data to log
   */
  info(module: string, message: string, data: unknown = null) {
    if (shouldLog('info')) {
      if (data !== null && data !== undefined) {
        console.log(`[${module}] ${message}`, data);
      } else {
   ...
   * @param {*} [data] - Optional data to log
   */
  warn(module: string, message: string, data: unknown = null) {
    if (shouldLog('warn')) {
      if (data !== null && data !== undefined) {
   ...
   * @param {*} [data] - Optional data to log
   */
  error(module: string, message: string, data: unknown = null) {
    if (shouldLog('error')) {
      if (data !== null && data !== undefined) {
